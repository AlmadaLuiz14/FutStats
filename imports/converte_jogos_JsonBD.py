        ...
        for rodada, jogos in data['jogosinfo'].items():
            for jogo in jogos:
                ...
                if count == 0:
                    ...
                    cursor.execute(
                        f"INSERT INTO {tabela} (Rodada, Info_Jogo, Time1, Brasao1, Time2, Brasao2, Placar, Local) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
                        (
                            int(rodada[6:]),
                            jogo.get('Info_Jogo', None),
                            jogo.get('Time1', None),
...
def converte_jogos():    
    salvar_jogos_do_json_no_postgresql("jogos_rodada.json", "jogos_rodada_2023")
