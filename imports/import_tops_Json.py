    ...
    if response.status_code == 200:
        ...
        if tabela_artilheiros is not None:
            ...
            for linha in tabela_artilheiros.find_all('tr'):
                ...
                # Encontra o brasão do time (URL da imagem do escudo)
                brasao_time = linha.find('img')
                if brasao_time:
                    dados_artilheiro['BrasaoTime'] = brasao_time['src']        
                ...
                # Encontra o nome do jogador
                apelido_jogador = linha.find_all('td')
                if len(apelido_jogador) >= 3:
                    dados_artilheiro['Apelido'] = apelido_jogador[2].text.strip()
